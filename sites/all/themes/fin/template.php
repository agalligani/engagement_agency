...
function _fin_add_css() {
  ...
  drupal_add_css($theme_path . '/css/fin.css');

  $skin_color = theme_get_setting('skin_color', 'fin');
 ...
 */
function fin_preprocess_page(&$vars) {
...
//        $vars['title'] = t('Blog');
  ...
  if (variable_get('theme_fin_first_install', TRUE)) {

    _fin_install();
  }

  $banners = fin_show_banners();
  $vars['slider'] = fin_banners_markup($banners);
}
...
function fin_field__field_image($variables) {
  $count = 0;
...
function fin_image_formatter($variables) {
